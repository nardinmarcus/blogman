/**
 * B7-G — shared state helpers for the batch-7 acceptance fixture tests
 * (issue #59).
 *
 * Two complementary ways to build a D1 state the reconciler can read:
 *
 *   - `createWranglerState()` + `applyB7Schema(state)` + `runD1` build a state
 *     purely through `wrangler d1 execute --local --persist-to <dir>`: the
 *     same channel `scripts/reconcile-b7-facts.mjs` reads. Used for the
 ...
 *     <dir>` reads. This lets the scenario tests drive the REAL batch-7
 *     kernels (idempotent Chrome clip + compare-then-confirm refresh) against
 *     a real D1 binding and then hand the resulting fact state to the
...
import { SOURCE_SYNC_DDL_STATEMENTS } from '@/lib/source-sync'
import { SOURCE_REFRESH_DDL_STATEMENTS } from '@/lib/source-refresh'

export { configPath, repoRoot, runD1, wranglerPath, spawnOk }

export const CANDIDATE = 'b7'.padEnd(40, '7')
  ...
  await db
    .prepare(
      `INSERT INTO migration_ledger (number, name, checksum, candidate_id)
       VALUES (1, 'b7-acceptance-fixture', '${'f'.repeat(64)}', '${CANDIDATE}')`,
    )
    .run()
}

/**
 * Apply the full batch-7 schema on a wrangler-persisted state through the very
 ...
 * source-sync (media_assets / source_media_mappings) + source-refresh
 * (proposals / records).
 */
export function applyB7Schema(state: string): void {
  ...
  spawnOk('source-sync ddl', [
    process.execPath, join(repoRoot, 'scripts', 'apply-source-sync-ddl.mjs'),
    '--local', '--persist-to', state, '--database', 'DB', '--config', configPath,
  ])
  spawnOk('source-refresh ddl', [
    process.execPath, join(repoRoot, 'scripts', 'apply-source-refresh-ddl.mjs'),
    '--local', '--persist-to', state, '--database', 'DB', '--config', configPath,
  ])
}
...
export function createWranglerState(): string {
  const dir = mkdtempSync(join(tmpdir(), 'blogman-b7-state-'))
  stateDirs.push(dir)
  return dir
}
 ...
 * batch-7 schema is applied through the ledger + the DDL statements the
 * modules ship.
 */
export async function createKernelContext(dir = createWranglerState()): Promise<KernelContext> {
  ...
  await seedLedger(db)
  // Miniflare-only accommodation (same as the B6 fixtures): the ledger's FTS
  ...
  for (const stmt of SOURCE_SYNC_DDL_STATEMENTS) await db.prepare(stmt).run()
  for (const stmt of SOURCE_REFRESH_DDL_STATEMENTS) await db.prepare(stmt).run()
...
/** Recompute a clip's derived article identity (draft_ref / slug) from a URL. */
export function clipDerivedIdentity(
  canonicalUrl: string,
): { draftRef: string; slug: string; identitySha256: string } {
  const identitySha256 = sha256(canonicalUrl)
  return { draftRef: `clip:${identitySha256}`, slug: `clip-${identitySha256.slice(0, 12)}`, identitySha256 }
}
...
export function runReconcileB7(
  state: string,
  report: string,
  extra: string[] = [],
): ReconcileResult {
  const result = spawnSync(process.execPath, [
    '--import', 'tsx', join(repoRoot, 'scripts', 'reconcile-b7-facts.mjs'),
    '--local', '--persist-to', state, '--database', 'DB', '--config', configPath,
    '--report', report, ...extra,
...
export async function cleanupB7State(): Promise<void> {
