...
export function AiProviderManager({ usageCounts }: { usageCounts?: Record<number, number> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {deleteTarget && (() => {
        ...
        if (isLastProfile) {
          return (
            <Modal
              ...
              title="无法删除"
              description={`「${deleteTarget.name}」是当前唯一的文本模型配置，删除后将没有可用配置，因此无法删除。请先新增其他配置。`}
              type="info"
            />
          )
        }
        ...
        if (usageCount > 0) {
          impactParts.push(`${usageCount} 个提示词绑定了此配置，删除后将回落到默认配置。`)
        }
