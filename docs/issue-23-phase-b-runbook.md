...
Stop on any remote plan failure or unexpected pending migration. In that case export, double restore, upload, migrations 001–006, CAS2, traffic, smoke/reconciliation, and T0 must all remain at attempt count `0`. The former query-7 blocker is superseded only for migration `001`, frozen `001_initial_schema.baseline.sql` (SHA-256 `b3f61982cc36ff2c88d7b4330dd304ef075b5c5c34debf4499671c33ae2b6540`) statement `3` (SHA-256 `c61b390568cafc468c6adbbff5b78d08dd5d18a544d917fbc06c043393e3c7bd`): `001_initial_schema.remote.baseline.sql` (SHA-256 `a3d4834018b7124c27ba82231c95239ce0092cd44a3b3959aec060b86accde28`) supplies exactly three smaller equivalent probes. Before the first Wrangler call, the runner verifies the migration number/name, full baseline SHA, source ordinal/SHA, replacement SHA/header and statement count. Each probe still uses remote `--command`, SELECT/WITH validation, `EXPLAIN` opcode proof, private mode-`0600` stdout/stderr/debug files, the fixed 300-second timeout and no retry; matching schema fingerprints are required immediately before and after the three-probe suite. Local plans and every other sidecar retain the original statements and opcode proof. Missing or drifted proof stops without fallback.
