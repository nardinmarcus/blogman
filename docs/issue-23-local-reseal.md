...
- `approval-packet.json` — `blogman-issue-23-approval-packet/v3`
- `pre-cas-bindings.json` — `blogman-issue-23-pre-cas-bindings/v3`
- `package-manifest.json` — `blogman-issue-23-package-manifest/v3`
...
into PRE-CAS, and binds all three into the package manifest. The v3 approval
scope ends at `T0 event acceptance`; PRE-CAS also binds the exact baseline D1
database UUID. Historical v2 fixtures remain valid byte-for-byte for read-only
validation and have `acceptance_authority=false`.
...
`blogman-issue-23-local-reseal-request/v2`, validated by
`blogman-issue-23-local-reseal-request-v2.schema.json`. Historical request/v1
bytes remain valid under their original schema, but current `seal` and `verify`
reject them as stale. The current request binds:
...
npm run issue-23:reseal -- validate --package /absolute/path/sealed-t0-or-historical-v2
```
...
file byte-for-byte. Only the coherent current v3 tuple is eligible here. A
coherent historical v2 tuple remains accepted by `validate --package` as
`state=valid-historical, acceptance_authority=false`, but current `verify`
rejects it as stale; mixed v2/v3 tuples are rejected. The tool does not query GitHub: the request's quick-CI
