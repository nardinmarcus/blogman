# Issue #23 local v2 reseal

`scripts/issue-23-reseal.mjs` is the repository-owned generator and verifier for
the local-only Issue #23 v2 quartet:

- `preflight-candidate.json` — `blogman-local-preflight-candidate/v2`
- `approval-packet.json` — `blogman-issue-23-approval-packet/v2`
- `pre-cas-bindings.json` — `blogman-issue-23-pre-cas-bindings/v2`
- `package-manifest.json` — `blogman-issue-23-package-manifest/v2`
...
into PRE-CAS, and binds all three into the package manifest. Historical v2
fixtures remain valid byte-for-byte.

`canonical_long_migration_runner` accepts the historical `unverified` object
or the current verified object `{state:"passed",passed:46,failed:0}`. No v2
top-level field changes. The package manifest retains the historical
`github_ci:"pending"` string. GitHub run/job details are external seal
prerequisites and are not represented as self-contained v2 evidence.

## Frozen input

The seal request must be canonical
`blogman-issue-23-local-reseal-request/v1`, validated by
`blogman-issue-23-local-reseal-request-v1.schema.json`. It binds:
...
npm run issue-23:reseal -- seal \
  ...
  --output /private/issue-23-sealed-v2/package
```

`/private/issue-23-sealed-v2` must not exist before this command.
...
npm run issue-23:reseal -- validate --package /absolute/path/sealed-v2
```
...
npm run issue-23:reseal -- verify \
  ...
  --repo /absolute/path/to/blogman \
  --artifacts /private/issue-23/build \
  --package /private/issue-23/sealed-v2
```

`verify` regenerates the expected quartet in memory and compares every package
file byte-for-byte. An internally valid old lineage therefore cannot verify
against a new candidate. The tool does not query GitHub: the request's quick-CI
