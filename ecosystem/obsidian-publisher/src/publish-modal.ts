...
export class PublishModal extends Modal {
  private plugin: BlogmanPublisher;
  ...
  async onOpen() {
    ...
    modalEl.addClass("blogman-publish-modal");
    this.injectStyles();
    ...
    const loadingEl = contentEl.createDiv({
      cls: "blogman-center-view",
    });
    loadingEl.createDiv({ cls: "blogman-spinner" });
    loadingEl.createDiv({
      text: "正在加载分类...",
      cls: "blogman-center-text",
    });
  ...
  private injectStyles() {
    const styleId = "blogman-modal-styles";
    if (document.getElementById(styleId)) return;

    const style = document.createElement("style");
    style.id = styleId;
    style.textContent = `
      .blogman-publish-modal {
        width: 520px;
        max-width: 90vw;
      }
      .blogman-center-view {
      ...
      .blogman-spinner {
        ...
        border-radius: 50%;
        animation: blogman-spin 0.6s linear infinite;
      }
      @keyframes blogman-spin {
  ...
  private renderProgress(message: string) {
    ...
    const wrapper = contentEl.createDiv({ cls: "blogman-center-view" });
    wrapper.createDiv({ cls: "blogman-spinner" });
  ...
  private renderSuccess(result: PublishResult) {
    ...
    wrapper.createDiv({ cls: "blogman-result-icon", text: "\u2713" });

    const heading = wrapper.createEl("h3", {
      text: "发布成功",
      cls: "blogman-result-heading",
    });
    heading.style.color = "var(--color-green)";

    // Title callout
    const callout = wrapper.createDiv({
      cls: "blogman-callout-box blogman-callout-success",
    });
    callout.createDiv({
    ...
    if (result.totalFiles > 0) {
      ...
      wrapper.createDiv({ cls: "blogman-stats-line", text: statsText });
    }
  ...
  private renderError(errorMsg: string) {
    ...
    // Error callout
    const callout = wrapper.createDiv({
      cls: "blogman-callout-box blogman-callout-error",
    });
